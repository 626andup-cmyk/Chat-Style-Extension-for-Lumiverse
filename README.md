...
- `ctx.ui.registerInputBarAction(...)` (toggle button + live queue-count label, in the input bar's "Extras" popover, free/no permission) — `frontend-api/ui-placement/`
- `ctx.ui.createFloatWidget(...)` (chromeless, extension-styled panel for the actual composer) — `frontend-api/ui-placement/`
...
The queue button lives in the chat input bar's "Extras" popover (grouped
under this extension's name) and its label doubles as a live counter, e.g.
`Queue (3)`. Clicking it toggles a small draggable panel (the float widget)
where you type and stage message chunks.
